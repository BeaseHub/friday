...
const Workspace = () => {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `${API_URL.replace(/^http/, "ws")}/ws/user_${userId}_conversations`
    );
    ...
    ws.onmessage = async (event) => {
      try {
        ...
        if (parsed.event === "new_conversation") {
          ...
          const { userId, token } = getAuthUserAndToken();
          const updatedConversations = await getConversationsByUser(
            userId,
            token
          );
  ...
  useEffect(() => {
    ...
    setMessages(
      selectedConversation.messages.map((msg) => ({
        sender: msg.is_systen
          ? "system"
          : selectedConversation.user_id === getAuthUserAndToken().userId
          ? "user"
          : "Unknonwn",
        content: msg.content,
        file: msg.file_path
          ? `${API_URL}/${msg.file_path.replace(/^\/+/, "")}`
          : null,
    ...
    // Connect to the WebSocket room for the selected conversation
    const ws = new WebSocket(
      `${API_URL.replace(/^http/, "ws")}/ws/conversation_${
    ...
    >
      ...
      <div>
        <elevenlabs-convai
          ...
          dynamic-variables={JSON.stringify({
            user_id: getAuthUserAndToken().userId,
            user_email: getAuthUserAndToken().userEmail,
          })}
        ></elevenlabs-convai>
      </div>
      ...
      <div className="h-[calc(100vh-88px)] flex">
        ...
        <div className="flex-1 flex flex-col">
          {/* Chat Header */}
          <div
            className={`border-b p-6 shadow-sm ${
              isDarkMode
                ? "bg-gray-900 border-gray-700"
                : "bg-white border-gray-200"
            }`}
          ...
          >
            {messages && messages?.length > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center p-4 sm:p-8 text-center w-full max-w-full overflow-hidden">
                <div
                  className={`w-32 h-32 rounded-full flex items-center justify-center mb-6 border-4 relative overflow-hidden ${
                    isDarkMode
                ...
                >
                  ...
                  {recording ? (
                    ...
                    <>
                      ...
                      ></div>
                      <div
                        className={`absolute inset-2 rounded-full animate-pulse opacity-5 animation-delay-500 ${
                ...
                >
                  {t("welcomeAiWorkspace")}
                </h2>
                <p
                  className={`mb-6 sm:mb-8 text-center w-full px-4 max-w-md ${
                    isDarkMode ? "text-gray-400" : "text-gray-600"
                  }`}
                >
          ...
          >
            ...
            {file && (
              <div className="flex flex-col items-center mb-2">
                ...
                <span className="mt-1 text-[11px] text-gray-600 max-w-[80px] truncate text-center">
                  {file.name}
                </span>
              </div>
            )}
