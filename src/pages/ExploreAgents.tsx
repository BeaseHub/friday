...
const ExploreAgents = () => {
  ...
  const { agents, filteredAgents, selectedAgents } = useAppSelector(
    (state) => state.agent
  );
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Une erreur est survenue lors de l'activation de votre espace. Veuillez réessayer.",
        variant: "destructive",
      });
      setProcessing(false);
      return;
      ...
      <div className="min-h-screen" style={{ background: "#F9F5F0" }}>
        ...
        <main className="container mx-auto px-6 py-8 ">
          <div className="mb-8">
            ...
            <p className="text-gray-600 mb-6">{t("viewActive")}</p>
                        ...
                        <div className="w-12 h-12 rounded-lg flex items-center justify-center bg-gray-100">
                          {agent.image_path ? (
                            <img
                              src={
                                agent.image_path
                                  ? `${API_URL}/${agent.image_path.replace(
                                      /^\/+/,
                                      ""
                                    )}`
                                  : ""
          ...
          {selectedAgents.length > 0 && (
            <div
              className="fixed bottom-0 left-0 right-0 p-6 shadow-lg"
              style={{
                background: "linear-gradient(to right, black, #686464)",
